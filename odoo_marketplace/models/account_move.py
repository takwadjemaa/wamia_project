...
class AccountMove(models.Model):
    ...
    @api.model
    def calculate_commission(self, list_price, seller_id,delivery, prod_delivery,  invoice_line_obj):
        price_unit = 0
        if not prod_delivery:
            if invoice_line_obj:
                seller_obj = self.env["res.partner"].browse(seller_id)
                commission = seller_obj.get_seller_global_fields('commission')
                comm_boy_delivery = delivery.product_id.list_price
    ...
    def create_seller_invoice_new(self):
        for invoice_obj in self:
            print(invoice_obj.invoice_line_ids)
            sellers = {"seller_ids": {}}
            if invoice_obj.move_type in ['out_invoice', 'out_refund']:
                for invoice_line_obj in invoice_obj.invoice_line_ids:
                    #Begin Custom*************************************************************************
                    delivery_prod = invoice_line_obj.product_id

                    delivery = self.env["delivery.carrier"].search(
                        [('product_id', '=', delivery_prod.id)])
                    prod_delivery = invoice_line_obj.sale_line_ids.filtered('is_delivery')
                    print("prod_delivery", prod_delivery)

                    # sale_order = self.env["sale.order"].search([('name', '=', invoice_obj.payment_reference)])
                    # print(sale_order)
                    # for s in sale_order.order_line:
                    #     print(s.product_id)
                    #     if s.product_id.name == "Gift Voucher":
                    #         print("voucher")
                    #         print(abs(s.price_unit))
                    #
                    ...
                    if seller_id:
                        seller_amount = self.calculate_commission(invoice_line_obj.price_total, seller_id,delivery, prod_delivery, invoice_line_obj)
                ...
                sellers.update({
                    ...
                    "description": _("Order Invoice Payment") if invoice_obj.move_type == "out_invoice" else _(
    ...
    def create_seller_payment_new(self, sellers_dict):
        ...
        # voucher_product_id = self.env['ir.default'].sudo().get('res.config.settings',
        #                                                        'wk_coupon_product_id')
        # invoice_id = sellers_dict.get('invoice_id', False)
        # invoice_obj = self.browse(int(invoice_id))
        # # if invoice_obj and invoice_obj.state in ['out_invoice', 'out_refund']:
        # voucher_inv_line = invoice_obj.invoice_line_ids.filtered(
        #         lambda l: l.product_id.id == voucher_product_id)
        # order = self.env["sale.order"].search([('name', '=', invoice_obj.payment_reference)], limit=1)
        ...
        #             voucher_seller_dict = sellers_dict.get("seller_ids")[
        #                 voucher_obj.marketplace_seller_id.id]
        #
        ...
        if sellers_dict:
            ...
            for seller in sellers_dict["seller_ids"].keys():
                ...
                for amount in sellers_dict["seller_ids"][seller]["invoice_line_payment"]:
                    total_amount += amount
                mp_currency_obj = self.env["res.currency"].browse(
                    self.env['ir.default'].get('res.config.settings', 'mp_currency_id')) or self.env.user.currency_id
                vals.update({
    ...
    def post(self):
        not_paid_invoices = self.filtered(
        ...
        not_paid_invoices.filtered(
            lambda move: move.payment_state in ('paid', 'in_payment')).mp_post_action_invoice_paid()
        return result
...
class AccountInvoiceLine(models.Model):
    ...
    # seller_payment_ids2 = fields.One2many("seller.payment", "invoice_id", string="Seller Payment")
