...
class SellerPayment(models.Model):
    ...
    # test_field_seller_payment=fields.Float("hhhh", compute="_get_information")
    #
    #
    # def _get_information(self):
    #     print(self.name)
    #     print(self.invoice_id.name)
    #     self.test_field_seller_payment=1.0
    #     invoice=self.env["account.move"].search([('name','=',self.invoice_id.name)])
    #     print(type(invoice.payment_reference))
    #     sale_order=self.env["sale.order"].search([('name','=',invoice.payment_reference)])
    #     print(sale_order)
    ...
    def action_invoice_register_payment(self):
        return self.env['account.payment'].with_context(active_ids=self.ids, active_model='seller.payment', active_id=self.id).action_register_payment()


    @api.model
    def create(self, vals):
        print("create seller payment")
    ...
    def copy(self, default=None):
        raise ValidationError(_("You can not duplicate seller payment."))
