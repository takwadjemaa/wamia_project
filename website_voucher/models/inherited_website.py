...
class Website(models.Model):
	...
	def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False, context=None):
		...
		order_total_price = 0
		voucher_dict = request.session.get('secret_key_data')
		if voucher_dict !=None and voucher_dict.get('coupon_id'):
			coupon_id = voucher_dict.get('coupon_id')
			voucher_obj = self.env['voucher.voucher'].sudo().browse(coupon_id)
			if hasattr(sale_order,'order_line'):
				...
				self.set_voucher_sale_order_price(sale_order, voucher_obj,voucher_product_id, selected_prod_percent_price,coupon_id,order_total_price)
		return sale_order
